...
#[allow(dead_code)]
pub async fn build_sample_csharp_project() -> Result<BuiltCSharpProjectOutput, CSharpError> {
    let path_to_csproj_file = SAMPLE_CSPROJ_PATH;
    addin_server_interactor::csharp::build_and_get_dlls_for_project(path_to_csproj_file).await
}